...
// 6. Add new JS before </script></body> - use split/join to avoid $ replacement issues
const newJS = `
// === AXIARCH UPGRADES ===
// Exit popup
...
async function submitPopupEmail(){const em=document.getElementById('popupEmail').value.trim();if(!em||!em.includes('@'))return;try{await fetch('/.netlify/functions/subscribe',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({email:em})});}catch(e){}closePopup();const m=document.getElementById('signup-msg');if(m){m.style.color='#00e87b';m.textContent="You are in! Picks at 8:30 AM ET.";}}
// Telegram sticky
(function(){if(!localStorage.getItem('ax_tg')){setTimeout(function(){document.getElementById('tgSticky').classList.add('show');},5000);}})();
function dismissTg(){document.getElementById('tgSticky').classList.remove('show');localStorage.setItem('ax_tg','1');}
// Theme toggle
function toggleTheme(){var on=document.body.classList.toggle('light-mode');document.getElementById('themeToggle').textContent=on?'\\u2600\\uFE0F':'\\uD83C\\uDF19';localStorage.setItem('ax_theme',on?'light':'dark');}
(function(){if(localStorage.getItem('ax_theme')==='light'){document.body.classList.add('light-mode');var b=document.getElementById('themeToggle');if(b)b.textContent='\\u2600\\uFE0F';}})();
// Mobile nav
function toggleMobileMenu(){document.querySelector('.nav-links').classList.toggle('open');}
// Ticker modal
function openModal(sym,price,change,score,bull,bear){document.getElementById('mSym').textContent='\\u0024'+sym;document.getElementById('mPrice').textContent='\\u0024'+parseFloat(price).toFixed(2);var c=document.getElementById('mChg');c.textContent=(change>=0?'+':'')+change+'%';c.style.color=change>=0?'var(--accent-green)':'var(--accent-red)';document.getElementById('mScore').textContent=score;document.getElementById('mBull').textContent=bull+'%';document.getElementById('mBear').textContent=bear+'%';document.getElementById('mYahoo').href='https://finance.yahoo.com/quote/'+sym;document.getElementById('mReddit').href='https://www.reddit.com/search/?q='+encodeURIComponent('\\u0024'+sym)+'&sort=new';document.getElementById('tickerModal').classList.add('show');}
...
// Member counter
(function(){var el=document.getElementById('memberCount');if(!el)return;var n=0;var t=setInterval(function(){n=Math.min(n+21,1200);el.textContent=n.toLocaleString()+'+';if(n>=1200)clearInterval(t);},20);})();
// Service worker
if('serviceWorker' in navigator)window.addEventListener('load',function(){navigator.serviceWorker.register('/sw.js').catch(function(){});});
`;

// Insert before </script>
const scriptEndIdx = h.lastIndexOf('</script>');
h = h.slice(0, scriptEndIdx) + newJS + h.slice(scriptEndIdx);
